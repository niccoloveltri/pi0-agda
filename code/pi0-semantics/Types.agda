...
data sem-μ {n : ℕ} (F : Fin n → Set) : Ty (suc n) → Set 

sem-Ty F (var i) = F i
sem-Ty F 𝟘 = ⊥
sem-Ty F 𝟙 = ⊤
sem-Ty F (A ⊕ A') = (sem-Ty F A) ⊎ (sem-Ty F A')
sem-Ty F (A ⊗ A') = (sem-Ty F A) × (sem-Ty F A')
sem-Ty F (μ A) = sem-μ F A

data sem-μ F where
  sup : ∀ {A} → sem-Ty F (sub A (μ A)) → sem-μ F A 
...
mutual
  ...
  data sem-μ {n : ℕ} (F : Fin n → Set) : Ty (suc n) → Set where
    sup : ∀ {A} → sem-Ty F (sub A (μ A)) → sem-μ F A 
-}
