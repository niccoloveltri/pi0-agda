...
the one in inverse categories [Kastl'79, Cockett and Lack'02].
...
\begin{frame}
  ...
  \item Then take:
    ...
    \end{array}
    \]
  \item The function $\iterD$ is compatible with weak bisimilarity,
    and satisfies the iteration axioms up to ${\approx}$. It lifts to
    an iterator $\iterDapprox$ in $\Dapprox$.
...
\begin{frame}
  ...
  \begin{itemize}
    \item From an iterator:
      ...
      we get a trace:
      \ctikzfig{trace}
      where $f = \copair {\mapL f}{\mapR f}$.
    \item Construction equivalent to the one in [Hasegawa'97].
...
\begin{frame}
  ...
  \item It remains to show that the trace of $\Dapprox$ restricts to
    $\Inv\Dapprox$.
  \item If $f : A + C \to \Delay \,(B + C)$ (\red{wrong type here, introduce notation for morphisms in $\Dapprox$}) is a partial isomorphism
    with partial inverse $g$, then the trace of $f$ is also a partial
    isomorphism with the trace of $g$ as partial inverse.
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{The orbit}
  \begin{itemize}
  \item The orbit of $x : A + C$ wrt. $f : A + C \to \Delay \,(B + C)$
    is the following inductively defined type family:
    \[
    \infer{\done\,p : \Orb f x y}{p : f \,x \approx \now \,y} \quad
    \infer{\mynext\,p \,q : \Orb f x y}{p : f \,x \approx \now
      \,(\inr\,c) & q : \Orb f {(\inr\,c)} y}
    \]
  \item Therefore: (\red{introduce $\traceD$ before})
    \[
    \traceD\,f\,a \approx \now\,b \leftrightarrow \Orb f{(\inl\,a)} {(\inl\,b)}
    \]
  \item Proof of dagger trace reduces to: (\red{introduce $\daggerD$ before})
    \[
...
i.e. partial isomorphisms:
  ...
  \item Term equivalences of \Pio\ are modelled as morphism equalities
    in $\Inv\Dapprox$, i.e. proofs of weak bisimilarity between two
    morphisms:
    \[
