...
via the trace combinator), and showed that these equivalences could be
...
which in particolar disallow the specification of a trace
operator. Consequently, it is necessary to move to a setting
...
%The binary product of $A$ and $B$ in $\Dapprox$ is given by $A + B + A \times B$. 
...
definition. The Elgot monad structure of $\mathsf{PMC}$ follows from its
