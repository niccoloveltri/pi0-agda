...
logically equivalent ways \cite{ChapmanUV19}. Here we consider a
coinductive formulation employed e.g. in \cite{Danielsson18}.
...
By abuse of notation, we use here $\cdot \times \cdot$ (respectively $\cdot +
\cdot$) to refer to the product (respectively coproduct) in $\Dapprox$ even
though it fails to be a product (respectively coproduct) in $\Inv\Dapprox$.
However, both of these are symmetric monoidal products in $\Inv\Dapprox$, so
their use as objects of $\Inv\Dapprox$ in the interpretation above is justified.
...
The notion of computable partiality offered the $\Delay$ monad and its
restriction category structure is well
studied~\cite{VeltriPhD,UustaluV17,ChapmanUV19}. Indeed, though it could be
established from previous work (see also \cite{KaarsgaardAG17}) that this
category was traced, it was unclear whether the trace preserved partial
isomorphisms -- a crucial point in its applicability to interpreting \Pio. The
presented solution further avoids CPO-enrichment (as otherwise used in both
\cite{UustaluV17} and \cite{KaarsgaardAG17}), and its issues in relation to
formalization, in favor of a more direct definition of the trace from the Elgot
iterator.
...
computation steps needed to perform $\trace (\dagger f)$ is \emph{precisely}
the same as what is needed to perform $\trace~f$ on any input. Since the
